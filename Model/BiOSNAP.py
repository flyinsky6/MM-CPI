...
def load_compound_features():
    ecfp = torch.tensor(np.load('D:\研究生\加特征数据\BioSNAP化合物四种特征完整版\BioSNAP（ECFP）完整.npy'), dtype=torch.float32)
    extfp = torch.tensor(np.load('D:\研究生\加特征数据\BioSNAP化合物四种特征完整版\BioSNAP（Extfp）完整.npy'), dtype=torch.float32)
    maccs = torch.tensor(np.load('D:\研究生\加特征数据\BioSNAP化合物四种特征完整版\BioSNAP（MACCS）完整.npy'), dtype=torch.float32)
    mol2vec = torch.tensor(np.load('D:\研究生\加特征数据\BioSNAP化合物四种特征完整版\BioSNAP（Mol2Vec）完整.npy'), dtype=torch.float32)
...
def load_protein_features():
    aac = torch.tensor(pd.read_csv(
        'D:\研究生\加特征数据\BioSNAP蛋白质三种特征完整版\BioSNAP蛋白质去重（AAC）完整版.csv', header=None).values, dtype=torch.float32)
    ctdc = torch.tensor(pd.read_csv(
        'D:\研究生\加特征数据\BioSNAP蛋白质三种特征完整版\BioSNAP蛋白质去重（CTDC）完整版.csv', header=None).values, dtype=torch.float32)
    gaac = torch.tensor(pd.read_csv(
        'D:\研究生\加特征数据\BioSNAP蛋白质三种特征完整版\BioSNAP蛋白质去重（GAAC）完整版.csv', header=None).values, dtype=torch.float32)
...
# 加载数据
compounds, proteins, labels = load_human_dataset('D:\研究生\加特征数据\BioDrugbank数据\BioSNAP.txt')
