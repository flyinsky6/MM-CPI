...
def load_compound_features():
    ecfp = torch.tensor(np.load('MM-CPI/Feature/celegans化合物四种特征完整版/ECFP完整.npy'), dtype=torch.float32)
    extfp = torch.tensor(np.load('MM-CPI/Feature/celegans化合物四种特征完整版/Extfp完整.npy'), dtype=torch.float32)
    maccs = torch.tensor(np.load('MM-CPI/Feature/celegans化合物四种特征完整版/MACCS完整.npy'), dtype=torch.float32)
    mol2vec = torch.tensor(np.load('MM-CPI/Feature/celegans化合物四种特征完整版/mol2vec完整.npy'), dtype=torch.float32)
    return ecfp, extfp, maccs, mol2vec

def load_protein_features():
    aac = torch.tensor(pd.read_csv(
        'MM-CPI/Feature/celegans蛋白质三种特征完整版/celegans（AAC）完整.csv', header=None).values, dtype=torch.float32)
    ctdc = torch.tensor(pd.read_csv(
        'MM-CPI/Feature/celegans蛋白质三种特征完整版/celegans（CTDC）完整.csv', header=None).values, dtype=torch.float32)
    gaac = torch.tensor(pd.read_csv(
        'MM-CPI/Feature/celegans蛋白质三种特征完整版/celegans（GAAC）完整.csv', header=None).values, dtype=torch.float32)
...
# 加载数据（路径与你一致）
# =========================
compounds, proteins, labels = load_human_dataset('MM-CPI/Dataset/celegans.txt')
