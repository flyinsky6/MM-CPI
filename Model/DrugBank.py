...
def load_compound_features():
    ecfp = torch.tensor(np.load('/Users/lizhiqiang/Desktop/研究生/数据/Drugbank化合物四种特征完整版/DrugBank（ECFP）完整.npy'), dtype=torch.float32)
    extfp = torch.tensor(np.load('/Users/lizhiqiang/Desktop/研究生/数据/Drugbank化合物四种特征完整版/DrugBank（ExtFP）完整.npy'), dtype=torch.float32)
    maccs = torch.tensor(np.load('/Users/lizhiqiang/Desktop/研究生/数据/Drugbank化合物四种特征完整版/DrugBank（MACCS）完整.npy'), dtype=torch.float32)
    mol2vec = torch.tensor(np.load('/Users/lizhiqiang/Desktop/研究生/数据/Drugbank化合物四种特征完整版/DrugBank（Mol2Vec）完整.npy'), dtype=torch.float32)
...
def load_protein_features():
    aac = torch.tensor(pd.read_csv(
        '/Users/lizhiqiang/Desktop/研究生/数据/Drugbank蛋白质三种特征完整版/DrugBank蛋白质去重（AAC）完整版.csv', header=None).values, dtype=torch.float32)
    ctdc = torch.tensor(pd.read_csv(
        '/Users/lizhiqiang/Desktop/研究生/数据/Drugbank蛋白质三种特征完整版/DrugBank蛋白质去重（CTDC）完整版.csv', header=None).values, dtype=torch.float32)
    gaac = torch.tensor(pd.read_csv(
        '/Users/lizhiqiang/Desktop/研究生/数据/Drugbank蛋白质三种特征完整版/DrugBank蛋白质去重（GAAC）完整版.csv', header=None).values, dtype=torch.float32)
...
# 加载数据
compounds, proteins, labels = load_human_dataset('/Users/lizhiqiang/Desktop/研究生/数据/DrugBank.txt')
...
with open(log_file, "w") as log:
    for epoch in range(num_epochs):
        ...
        # 计算训练损失、准确率、AUC、Precision、Recall、F1-score
        ...
        train_f1 = f1_score(all_labels, (np.array(all_preds) > 0.5).astype(int))

        # 记录训练结果
        log.write(
            f"Epoch {epoch + 1}/{num_epochs} - Train Loss: {train_loss:.4f}, Accuracy: {train_accuracy:.4f}, AUC: {train_auc:.4f}, Precision: {train_precision:.4f}, Recall: {train_recall:.4f}, F1: {train_f1:.4f}\n")
        print(
            f"Epoch {epoch + 1}/{num_epochs} - Train Loss: {train_loss:.4f}, Accuracy: {train_accuracy:.4f}, AUC: {train_auc:.4f}, Precision: {train_precision:.4f}, Recall: {train_recall:.4f}, F1: {train_f1:.4f}")
        ...
        # 计算验证损失、准确率、AUC、Precision、Recall、F1-score
        ...
        # 记录验证结果
        log.write(
            f"Epoch {epoch + 1}/{num_epochs} - Val Loss: {val_loss:.4f}, Accuracy: {val_accuracy:.4f}, AUC: {val_auc:.4f}, Precision: {val_precision:.4f}, Recall: {val_recall:.4f}, F1: {val_f1:.4f}\n")
        print(
            f"Epoch {epoch + 1}/{num_epochs} - Val Loss: {val_loss:.4f}, Accuracy: {val_accuracy:.4f}, AUC: {val_auc:.4f}, Precision: {val_precision:.4f}, Recall: {val_recall:.4f}, F1: {val_f1:.4f}")
        ...
        # 计算测试损失、准确率、AUC、Precision、Recall、F1-score
        ...
        test_f1 = f1_score(all_labels, (np.array(all_preds) > 0.5).astype(int))

        # 记录测试结果
        log.write(
            f"Epoch {epoch + 1}/{num_epochs} - Test Loss: {test_loss:.4f}, Accuracy: {test_accuracy:.4f}, AUC: {test_auc:.4f}, Precision: {test_precision:.4f}, Recall: {test_recall:.4f}, F1: {test_f1:.4f}\n")
        print(
            f"Epoch {epoch + 1}/{num_epochs} - Test Loss: {test_loss:.4f}, Accuracy: {test_accuracy:.4f}, AUC: {test_auc:.4f}, Precision: {test_precision:.4f}, Recall: {test_recall:.4f}, F1: {test_f1:.4f}")

# 保存模型
torch.save(model, "DrugBank.pth")