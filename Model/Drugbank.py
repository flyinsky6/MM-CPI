...
def load_compound_features():
    ecfp = torch.tensor(np.load('MM-CPI/Feature/Drugbank化合物四种特征完整版/DrugBank（ECFP）完整.npy'), dtype=torch.float32)
    extfp = torch.tensor(np.load('MM-CPI/Feature/Drugbank化合物四种特征完整版/DrugBank（ExtFP）完整.npy'), dtype=torch.float32)
    maccs = torch.tensor(np.load('MM-CPI/Feature/Drugbank化合物四种特征完整版/DrugBank（MACCS）完整.npy'), dtype=torch.float32)
    mol2vec = torch.tensor(np.load('MM-CPI/Feature/Drugbank化合物四种特征完整版/DrugBank（Mol2Vec）完整.npy'), dtype=torch.float32)
    return ecfp, extfp, maccs, mol2vec

def load_protein_features():
    aac = torch.tensor(pd.read_csv(
        'MM-CPI/Feature/Drugbank蛋白质三种特征完整版/DrugBank蛋白质去重（AAC）完整版.csv', header=None).values, dtype=torch.float32)
    ctdc = torch.tensor(pd.read_csv(
        'MM-CPI/Feature/Drugbank蛋白质三种特征完整版/DrugBank蛋白质去重（CTDC）完整版.csv', header=None).values, dtype=torch.float32)
    gaac = torch.tensor(pd.read_csv(
        'MM-CPI/Feature/Drugbank蛋白质三种特征完整版/DrugBank蛋白质去重（GAAC）完整版.csv', header=None).values, dtype=torch.float32)
...
# 加载数据（路径与你一致）
# =========================
compounds, proteins, labels = load_human_dataset('MM-CPI/Dataset/DrugBank.txt')
